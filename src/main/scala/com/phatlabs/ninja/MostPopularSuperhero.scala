...
object MostPopularSuperhero {
  ...
  def main(args: Array[String]) {
    ...
    val names = sc.textFile("src/main/resources/SparkScala/marvel-names.txt")
    val namesRdd = names.flatMap(parseNames)
    
    // Load up the superhero co-apperarance data
    val lines = sc.textFile("src/main/resources/SparkScala/marvel-graph.txt")
    ...
    val flippedAndSortedAsc = totalFriendsByCharacter.map( x => (x._2, x._1) ).sortByKey()
    val flippedAndSortedDsc = totalFriendsByCharacter.map( x => (x._2, x._1)).sortByKey(ascending = false)
...
//    println(s"$mostPopularName is the most popular superhero with ${mostPopular._1} co-appearances.")
    ...
    val top10 = flippedAndSortedDsc.take(10).map(x => (namesRdd.lookup(x._2).head, x._1))
    val bottom10 = flippedAndSortedAsc.take(10).map(x => (namesRdd.lookup(x._2).head, x._1))

    println("Top 10 Superheros")
    top10.foreach(println)
    println("-----------------------")
    println("Top 10 Least Popular Superheros")
